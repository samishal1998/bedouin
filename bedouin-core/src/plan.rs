...
pub fn build(
    ...
    if rc_capable {
        // bedouin's own completion always lands in the drop-in tree, and for
        // zsh and bash the block below is what puts it on `fpath` or sources
        // it. Leave this conditional and the completion is written into a
        // directory nothing reads. fish finds its completions natively, so it
        // still only needs the directory when something else writes there.
        let writes_shell_files = cfg.shell != Shell::Fish
            || !cfg.aliases.is_empty()
            || cfg.packages.iter().any(|p| {
                !p.rc.is_empty()
                    || !p.path.is_empty()
        ...
        // bedouin's own comes first and is never declared: a tool that
        // configures your shell should not make you ask for its own
        // completion. The generator is the running binary, so it regenerates
        // when bedouin is upgraded -- the digest is over the argv, and the
        // path changes with the version only if it is versioned, so this is
        // best-effort rather than a guarantee.
        let mut wanted: Vec<(String, Vec<String>, bool)> = vec![(
            "bedouin".to_string(),
            vec![
                facts.exe.clone(),
                "completion-script".to_string(),
                cfg.shell.to_string(),
            ],
            false,
        )];
        for p in &cfg.packages {
            ...
            wanted.push((p.name.clone(), argv.clone(), changing));
        }
