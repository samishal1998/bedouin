...
mod tests {
    ...
    fn a_blocks_content_can_be_read_back_out_of_the_file() {
        ...
        assert_eq!(
            extract_block(&text, "zellij").unwrap().as_deref(),
            Some(content)
        );
        assert_eq!(extract_block(&text, "absent").unwrap(), None);
        assert_eq!(
            digest(&extract_block(&text, "zellij").unwrap().unwrap()),
            digest(content)
        );
    }
    ...
    fn the_source_snippet_is_a_no_op_for_fish() {
        ...
        assert!(
            zsh.contains("fpath="),
            "completions need fpath set before compinit"
        );
